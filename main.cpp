...
public:
  TracedThread(pid_t tid) : FsThread<TracedThread>(tid), pipe_fd(-1) {}

  bool can_see_path(std::string path, int at, int &error) {
    std::string canon = canon_path_at(tid, at, path);
    for (std::vector<std::string>::const_iterator i = conf_see.begin(),
                                                  end = conf_see.end();
         i != end; i++)
      if (in_dir(*i, canon))
        return true;
  ...
  bool can_write_path(std::string path, int at, int &error) {
    ...
    for (std::vector<std::string>::const_iterator i = conf_write.begin(),
                                                  end = conf_write.end();
         i != end; i++)
      if (in_dir(*i, canon))
        return true;
  ...
  bool should_replace_stat(std::string path, struct stat &st, int at,
                           int flags) {
    if (path.compare(0, strlen("http:/"), "http:/") == 0 &&
        path.compare(0, strlen("http://"), "http://") != 0)
      path = "http://" + path.substr(strlen("http:/"));
    else if (path.compare(0, strlen("https:/"), "https:/") == 0 &&
             path.compare(0, strlen("https://"), "https://") != 0)
      path = "https://" + path.substr(strlen("https:/"));
    for (std::vector<std::string>::const_iterator i = conf_http.begin(),
                                                  end = conf_http.end();
         i != end; i++)
  ...
  bool should_replace_path(std::string path, std::string &newpath, int at,
                           int flags) {
    if (path.compare(0, strlen("http:/"), "http:/") == 0 &&
        path.compare(0, strlen("http://"), "http://") != 0)
      path = "http://" + path.substr(strlen("http:/"));
    ...
    for (std::vector<std::string>::const_iterator i = conf_http.begin(),
      ...
      if (path.compare(0, i->size(), *i) == 0) {
        ...
        if (pid == 0) {
          dup2(fd, 1);
          close(fd);
          execlp("curl", "curl", "--silent", "--globoff", "--", path.c_str(),
                 NULL);
          exit(1);
        }
...
void set_limits() {
  ...
  for (std::vector<std::string>::const_iterator i = conf_unsetenv.begin(),
                                                end = conf_unsetenv.end();
       i != end; i++)
    unsetenv(i->c_str());
  ...
  struct rlimit limit;
  for (std::vector<std::pair<int, rlim_t>>::const_iterator
           i = conf_rlimit.begin(),
           end = conf_rlimit.end();
       i != end; i++) {
...
int main(int argc, char **argv) {
  ...
  if (!pid) {
    ...
    if (argc >= 4) {
      main_process =
          sandbox.spawn_process(argv[3], argc - 3, argv + 3, set_limits);
    } else {
      std::vector<char *> av;
      for (std::vector<std::string>::iterator i = conf_arg.begin(),
                                              end = conf_arg.end();
           i != end; i++)
        av.push_back(&(*i)[0]);
      main_process = sandbox.spawn_process(conf_program.c_str(), av.size(),
                                           av.data(), set_limits);
    }
    close(in_pipe[0]);
    close(out_pipe[1]);
    feed_in = new std::thread(feed_data, fileno(stdin), in_pipe[1], true,
                              !conf_wakeup);
    feed_out =
        new std::thread(feed_data, out_pipe[0], fileno(stdout), false, false);
    sandbox.event_loop();
    exit(0);
  } else {
