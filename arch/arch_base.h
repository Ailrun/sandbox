...
protected:
  inline SyscallInfoBase(ThreadData &thread_, bool) : thread(thread_) {}
...
public:
  ...
  inline SyscallInfoBase(ThreadData &thread_) : thread(thread_) {
  ...
  uint8_t fetch_byte(void *addr) {
    long word = 0;
    thread.ptrace_peekdata((void *)((uintptr_t)addr & ~(sizeof word - 1)),
                           word);
  ...
  bool emplace_array(void *addr, std::vector<uint8_t> array) {
    ...
    while (write_addr < (char *)addr + array.size()) {
      ...
      if (++off == sizeof word) {
        ...
        if (write_addr < (char *)addr + array.size() &&
            write_addr > (char *)addr + array.size() - 8)
          if (!thread.ptrace_peekdata(write_addr, word))
            return false;
      }
    }
