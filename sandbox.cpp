...
void panic(std::string s) { throw std::runtime_error(s); }

void panic_errno(std::string s) { panic(s + ": " + strerror(errno)); }
...
void ThreadData::set_options() {
  if (!set_opts) {
    if (!ptrace_setoptions(PTRACE_O_EXITKILL | PTRACE_O_TRACECLONE |
